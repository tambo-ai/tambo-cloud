...
export const ActionBadge = memo(({ type }: ActionBadgeProps) => (
...
export const ToolCallBadge = memo(({ id, onHover }: ToolCallBadgeProps) => (
  <Badge
    variant="outline"
...
export const ToolCallCode = memo(
  ({ toolName, parameters }: ToolCallCodeProps) => {
    ...
    const paramString = parameters
      .map(
        (param) =>
          `${param.parameterName}: ${formatValue(param.parameterValue)}`,
      )
      .join(", ");
        ...
        {expanded ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="text-xs h-auto p-1 shrink-0"
            >
              Expand
            </Button>
          </div>
...
export const SuggestedActions = memo(({ actions }: SuggestedActionsProps) => (
