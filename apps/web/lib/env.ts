...
// Wrap env creation in a function so we can catch and log validation errors
const createValidatedEnv = () =>
  createEnv({
    extends: [vercel()],
    onValidationError: (issues) => {
      console.error("[env] (onValidationError) Validation failed — issues:");
      ...
      const presenceSummary = requiredKeys
        .map((k) => `${k}=${process.env[k] ? "set" : "missing"}`)
        .join(", ");
      console.error(
        `[env] (onValidationError) Required keys presence: ${presenceSummary}`,
      );
      // Must throw to satisfy the `never` return type and stop boot
    ...
    server: {
      ...
      // Freestyle Sandbox API key for AI Playground (server-side only)
...
let _env;
try {
  _env = createValidatedEnv();
} catch (e) {
  // Log structured details without leaking secret values
  if (e instanceof z.ZodError) {
    ...
    const presenceSummary = requiredKeys
      .map((k) => `${k}=${process.env[k] ? "set" : "missing"}`)
      .join(", ");
    console.error(`[env] Required keys presence: ${presenceSummary}`);
  } else {
    console.error("[env] Non-Zod error during env validation:", e);
  }
  throw e;
}
