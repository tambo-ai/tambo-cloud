...
I've been wanting to build an AI-powered spreadsheet for a while now, and honestly, Google's Gemini integration for Sheets hasn't impressed me yet. So I decided to see what we could do with Tambo.
...
<video src="https://github.com/user-attachments/assets/39d1b5b0-cff2-4a57-b825-f6ae9785eec4" controls />
...
Tambo's context helper feature made this super easy. I just write a function that formats the data, register it, and it automatically gets included with every message. I don't have to think about when or how to inject that context. It's just always there.
...
function formatSpreadsheetAsMarkdown(activeTab) {
  ...
  // Get column headers (A, B, C, etc.)
  const columnIds = columns
    .filter(col => col.columnId !== 'ROW_HEADER')
  ...
  for (let i = 1; i < rows.length; i++) {
    const row = rows[i];
    const cellValues = row.cells.map(cell => cell.text || cell.value || '');
    markdown += `| ${row.rowId} | ${cellValues.join(' | ')} |\n`;
  }
...
One interesting thing I discovered was how the AI learned to use the tools correctly. At first, it would make mistakes (passing flat arrays instead of 2D arrays, or using the wrong cell format). I added explicit error messages to help guide it, and the AI could actually self-correct at runtime. It would see the error, understand what went wrong, and retry with the correct format.

But self-correction isn't the end goal. It's a signal that something needs fixing. Even though your AI assistant can recover from mistakes, you should be watching for recurring tool errors and using them as feedback to improve your tool definitions. I'd copy the incorrect tool call, the error message, and what the AI eventually sent that worked, then feed all that back to Claude and ask it to update the tool descriptions, parameter schemas, or validation logic to prevent that mistake from happening in the first place. So over time, the tools got better and better, and the AI needed to self-correct less and less. It's a development pattern I hadn't really used before: using AI to help you write better prompts and tool definitions for itself, turning runtime errors into design improvements.
...
<video src="https://github.com/user-attachments/assets/958dbd48-1b7d-4471-93a0-62125a685587" controls />
...
I used additional context helpers for this. It just passes the selected range with every message. Since the AI already has the full spreadsheet state from the markdown context helper, it can figure out exactly what I'm referring to.
...
Spreadsheets need custom tools because they're arrays of rows containing arrays of cells. Custom tools let the AI work at the right level (individual cells and ranges) instead of manipulating nested structures.
...
Try it out: [https://github.com/michaelmagan/cheatsheet](https://github.com/michaelmagan/cheatsheet)
