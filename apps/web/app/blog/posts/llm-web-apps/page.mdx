...
date: "2025-11-07"
description: "Let LLMs control your web app"
...
When software understands natural language, we can be vague about our end goals. When we don’t know all the steps to get to our goal, or we don’t have a clear idea of what exactly our end goal looks like, we can have the LLM guide us towards it. When tools are available, it can even infer what actions to take and perform them for us.

After a few years of using LLM chat interfaces, though, it’s become clear that **natural language alone isn’t enough** for every use case.

## We Still Need UI

When we give an AI a vague goal, it needs to fill in the gaps and assume the steps to get there. We usually don’t want the AI to perform the actions based on guesses without asking us for confirmation. When an LLM or an agent proposes an action, a UI allows helps us understand the context and input edits much more easily than language.
...
When an AI can orchestrate text and UI components together into an interface tailored to our intent, the experience becomes both flexible and efficient.

## We Don’t Need to Generate UI

When people discuss “AI-driven UI,” the phrase generative UI often comes up.
...
Instead, we can let the LLM control pre-built UI components that would normally be placed somewhere on the page. Then the AI only has access to the components and functions it’s given by the developer.
...
- **Voice input:** Natural-language control doesn’t have to come from typing.
...
Then add a `<MessageInput>` component somewhere for users to submit messages, and render the output messages, including any used components or tool call information.

Tambo processes and stores the messages automatically.

With just those pieces, you can give your application a conversational interface powered by LLMs.

From there, you can go much deeper with Tambo: connect MCP servers, integrate shared context across users, or let Tambo interact with components already rendered on the page.
