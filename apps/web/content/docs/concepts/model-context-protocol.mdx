...
Server-side MCP integration allows you to configure MCP servers at the project level through the Tambo dashboard.
...
4. Enter the server URL and server type (SSE or StreamableHTTP), and click the save button.
5. Tambo will automatically detect if an MCP server requires authentication. If so, the "Begin Authentication" button will be shown. Click it to begin the authentication process.

Once configured, the MCP servers will be available to all agents in your project without any additional client-side setup.

**Note:** _When an MCP server is authenticated, the authenticatated user is currently shared across **all** users of the project. In the future Tambo will support per-user authentication._

If your server does not support OAuth-based authentication, you can add a custom header to the MCP server configuration.
...
Client-side MCP integration allows your application to connect to MCP servers that are accessible from the end user's browser.
