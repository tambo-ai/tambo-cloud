...
export default function Note({ title, content, createdAt }: NoteProps) {
  ...
  return (
    <div>
      <h3>{title}</h3>
      <p>{content}</p>
      <p>{propStatus["createdAt"].isSuccess && createdAt}</p>
    </div>
  );
}
```
...
- `isPending`: whether we have received any values for this prop.
- `isStreaming`: whether the we have started recieving values for this prop.
