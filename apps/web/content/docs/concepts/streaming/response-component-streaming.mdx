...
Tambo streams props into your components as they're generated, creating a dynamic experience where content appears progressively rather than waiting for completion.
...
### 3. Nullish Coalescing (Most Control)

```tsx title="nullish-coalescing.tsx"
const ProductCard = ({
  name,
  price,
  description,
  ...
  return (
    <div className="product-card">
      <h3>{name ?? "Loading product..."}</h3>
      <p className="price">
        {price !== undefined ? `$${price}` : "Loading price..."}
      </p>
