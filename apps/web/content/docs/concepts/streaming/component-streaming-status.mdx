...
The `useTamboStreamStatus` hook provides both global and per-prop streaming status tracking. When AI streams props into your components, not all properties arrive simultaneously - some props (like a title) might stream in quickly, while others (like complex content) take longer.
