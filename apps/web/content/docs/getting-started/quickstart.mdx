...
Download and run our generative UI chat template to get an understanding of using the fundamental features of Tambo. This application will show you how to build generative UI, integrate tools, send messages to Tambo, and stream responses.
...
Start the app and go to `http:localhost:3000` to start sending messages to Tambo!
...
Let's create and register a new component Tambo to give our AI chat a new feature.
...
You might notice that when using our added `RecipeCard` component above, Tambo generates recipe data completely from scratch. To make sure Tambo only generates recipes using the ingredients we actually have, we can add a tool to get the list of available ingredients.
...
This template app just scratches the surface of what you can build with Tambo. Instead of response components rendering in chat, you could have them render anywhere on screen. Add any custom logic as a tool to give Tambo capabilities. Integrate with MCP servers.
