...
When Tambo chooses a component to use, it will look at the component's `propsDefinition` to know what props to pass to the component, and how they should be structured.
...
registerComponent({
  ...
  propsDefinition: {
    meetings:
      "{id: string, title: string, date: string, location: string, attendees: string[]}[]",
  },
  associatedTools: [meetingsTool],
});
```
