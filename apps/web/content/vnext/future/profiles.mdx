...
User profiles in tambo allow you to store and manage user-specific information that helps personalize AI interactions. This feature enables maintaining context about users across sessions and improving the AI's ability to provide relevant responses.
...
### The useTamboProfile Hook

The `useTamboProfile` hook provides easy access to profile operations:
...
```typescript
const { status } = useTamboProfile(userId);
...
Through the main tambo context, you can access all profile functionality:

```typescript
const { profiles } = useTamboContext();
...
function UserProfileManager({ userId }: { userId: string }) {
  const { profile, operations, status } = useTamboProfile(userId);
