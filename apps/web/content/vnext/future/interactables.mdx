...
When a user says something like "add a row to the table" or "update the chart title", tambo will automatically identify which interactable they're referring to and update its state accordingly.
...
Register an interactable component by adding the `isInteractable` flag. This tells tambo that this component can be updated through conversation:
...
```ts
function TodoTable() {
  const { state, setState, mode } = useTamboMessage<TodoTableState>();
...
The `useTamboMessage` hook provides unified state management:
...
function CreateInteractiveThread() {
  const createThread = useTamboCreateThread();
...
function ThreadControls({ threadId }: { threadId: string }) {
  const { isInteractiveOnly, setInteractiveOnly } = useTamboInteractiveMode(threadId);
  return (
    <div>
      <Switch
...
function ThreadMessage({ messageId, threadId }: { messageId: string; threadId: string }) {
  const { setMessageInteractiveOnly } = useTamboInteractiveMode(threadId);
  const { component } = useTamboThreadComponent(messageId);
...
Interactables can be used within your own presentation layer. Here's an example of implementing a Canvas UI that manages layout while letting tambo handle component state:
