...
description: Understanding message threads as the foundation of tambo applications.
---
...
const { messages, operations, suggestions, components } = useTamboThread(threadId);

// Access global thread state and operations
const { threads, messages, components, suggestions } = useTamboContext();
...
} = useTamboContext().threads.operations;
```
...
const { generate } = useTamboContext().messages;
...
const {
  ...
  error,
  validationState,
} = useTamboContext().messages.status;
...
const threadsByContext = useTamboContext().threads.getByContext(contextId);
const allThreads = useTamboContext().threads.all;
const threadStates = useTamboContext().threads.state;
...
import { useTamboThread, useTamboContext } from "tambo-react";
import { type FormEvent, useState } from "react";

function BasicMessageThread({ threadId }) {
  const { messages, suggestions } = useTamboThread(threadId);
  const { generate } = useTamboContext().messages;
  const { isThinking, isLoading, error } = useTamboContext().messages.status;
