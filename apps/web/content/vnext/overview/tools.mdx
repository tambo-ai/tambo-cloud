...
const myTool1 = // tool definition
const myTool2 = // tool definition
registerTool(myTool1); // register a single tool
registerTools([myTool1, myTool2]); // register multiple tools at once
```
...
```tsx
const myWeatherTool: HydraTool = {
  // tool definition
};

hydraClient.registerComponent({
  name: "MyWeatherComponent",
  //other component definition fields...
  associatedTools: [myWeatherTool],
});
```
...
const myTool1: HydraTool = {
  //tool definition
};

addToolAssociation("MyWeatherComponent", myTool1); // will fail if there is no registered component with the name 'MyWeatherComponent'
```
