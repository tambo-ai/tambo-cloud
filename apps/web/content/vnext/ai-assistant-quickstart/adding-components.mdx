...
description: Learn how to create and register components with Hydra AI.
---
...
Now let's create our display components using the inferred types:
...
export const componentRegistry = createHydraComponentRegistry({
  CustomerNote: {
    ...
    description: "Displays a customer interaction note",
    propsSchema: customerNoteSchema,
    associatedTools: ["createNote", "updateNote"],
  },
  EmailPreview: {
    component: EmailPreview,
    description: "Displays an email preview generated from a note",
...
<Callout>
  The AI will use these components to display generated notes and email drafts.
  These are static components that are displayed in the chat, later we show how
  to maket the interactive.
</Callout>

## Adding to the Message Thread

Add the components to your message thread:
...
<Callout>
  The AI will automatically include components in messages with their Component
  and props ready to use.
</Callout>

<Callout title="Next Step">
  We'll add tools to fetch and manage lead data. Context for our notes.
</Callout>
