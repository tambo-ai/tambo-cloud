...
### 1. Update the Note Component to Use tambo State

Use `useTamboState` is a hook provided by tambo to manage the state of the component. It works just like React's `useState` hook, but it's synced with the AI backend.

**What's happening here:**

- Use the `useTamboState` hook to manage the state of the component
- State is automatically synced with the AI backend through tambo's context

```tsx title="src/components/InteractiveNote.tsx"
import { useTamboState } from "tambo-react";
