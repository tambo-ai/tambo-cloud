  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      if (user.id == requestedUserId) {
        this.logger.log(`[${correlationId}] User ${user.id} accessed their own data`);
        return true;
      }
      this.logger.warn(`[${correlationId}] User ${user.id} attempted to access data for user ${requestedUserId}`);
      return false;
    } catch (e) {
      this.logger.error(
