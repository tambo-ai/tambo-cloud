...
**🚨 KEY DIFFERENCE: Interactable components auto-register when mounted - DO NOT add them to `TamboProvider` components array or `config.ts`!**

Unlike regular components that must be registered in `TamboProvider` (via `lib/tambo/config.ts`), interactable components handle their own registration automatically when they mount.

**Where components go:**

- **Interactable components**: Pre-placed in your JSX, NOT in `config.ts`
- **Regular components**: Registered in `lib/tambo/config.ts` for Tambo to generate dynamically
...
| **Registration** | ✅ Auto-registers on mount                              | ❌ Must register in `lib/tambo/config.ts`                       |
...
| **Examples**     | API Key List, OAuth Settings, Tool Call Limit Editor    | Project Table, Daily Messages Chart, Thread Table               |

**Pro Tip**: If you want BOTH behaviors (pre-placed AND Tambo can generate new ones), register it in `lib/tambo/config.ts` too (advanced use case).

## Component Registration Guide

### Interactable Components (DO NOT register)

These components are pre-placed in your JSX and auto-register on mount:
...
**Action**: Simply use them in your JSX with `<InteractableComponentName ... />`

### Regular Components (MUST register in config.ts)

These components are dynamically generated by Tambo and must be in `lib/tambo/config.ts`:
...
**Action**: Add to `tamboRegisteredComponents` array in `lib/tambo/config.ts`
...
- **🚨 DO NOT register interactable components in `TamboProvider` components array or `config.ts`** - they auto-register on mount
- **🚨 DO NOT add interactable components to `tamboRegisteredComponents` in `lib/tambo/config.ts`** - only regular (Tambo-generated) components belong there
- Interactable components are pre-placed by you in JSX and control themselves
- Regular components (in config.ts) are dynamically generated by Tambo when needed
- If you want Tambo to both modify pre-placed instances AND generate new instances inline, then register the component normally in `TamboProvider` as well (advanced use case)
...
- [ ] **NOT** added it to `lib/tambo/config.ts` - only regular components go there
