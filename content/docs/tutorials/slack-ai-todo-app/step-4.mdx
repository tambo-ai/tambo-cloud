...
There are also a couple of loading components that we'll use to show while the "main" components are loading.

To register them with Hydra, we'll update the `hydra-config.ts` file we created earlier to make a `registerComponnet` call for each of them.
