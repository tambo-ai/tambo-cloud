...
export class ComponentDecisionService {
    ...
    private async handleNoComponentCase(decisionResponse: any, context: InputContext) {
        ...
        const noComponentResponse = await this.llmClient.complete([
            {
                role: "system",
                content: this.promptService.generateNoComponentPrompt(reasoning, context.availableComponents),
            },
            ...chatHistoryToParams(context.messageHistory),
        ]);
