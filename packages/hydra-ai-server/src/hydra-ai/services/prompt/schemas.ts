...
export const schema = z.object({
    ...
    suggestedActions: z
        .array(
            z.object({
                label: z.string().describe("The text to show for this action in a button. This should be a short, concise label that would be used in a button, and should describe what the action does."),
                actionText: z
                    .string()
                    .describe("The exact text that will be sent to the AI on the user's behalf when they select this action.")
            })
        )
        .max(3)
        .describe(
